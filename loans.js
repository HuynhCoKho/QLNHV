...
const LOAN_FIELDS = ['MÃ SỐ KV','MÃ KH','SỐ VBXN','NGÀY VBXN','KIM NGẠCH VAY','ĐỒNG TIỀN','FILE'];
...
function renderKhoanVay() {
  ...
  view.innerHTML = `
    ...
    <div class="card"><div class="table-wrap"><table>
      <thead><tr><th>Mã số khoản vay</th><th>Số VB xác nhận</th><th>Ngày VB</th><th>Kim ngạch vay</th><th>Đồng tiền</th><th>Lịch sử</th><th></th></tr></thead>
  ...
  const draw = () => {
    ...
    const sortedGroups = [...groups.entries()].sort((a,b)=>(khName(a[0])||a[0]).localeCompare(khName(b[0])||b[0],'vi'));
    ...
    body.innerHTML = sortedGroups.length ? sortedGroups.map(([ma, loans]) => {
      ...
      return `<tr class="group-row"><td colspan="7"><b>${esc(khName(ma)||'Chưa xác định khách hàng')}</b><span class="group-meta mono">${esc(ma)} · ${loans.length} khoản vay</span></td></tr>` +
        loans.map(r => `<tr class="clickable-row" data-loan="${esc(r['MÃ SỐ KV'])}">
          <td class="mono"><b>${esc(r['MÃ SỐ KV'])}</b></td><td>${esc(r['SỐ VBXN'])}</td>
          <td class="mono">${esc(fmtDateVN(r['NGÀY VBXN']))}</td><td class="num">${esc(fmtNum(r['KIM NGẠCH VAY']))}</td>
          <td class="mono">${esc(r['ĐỒNG TIỀN'])}</td><td><span class="badge badge-on">${loanHistory(r['MÃ SỐ KV']).length} văn bản</span></td>
    ...
    }).join('') : `<tr><td colspan="7"><div class="empty-state"><h3>Không có khoản vay phù hợp</h3></div></td></tr>`;
...
function showLoanHistory(maKV) {
  ...
  openModal(`Lịch sử khoản vay ${maKV}`, `<div class="loan-summary"><div><span>Doanh nghiệp</span><b>${esc(kh?kh.TenKhachHang:'—')}</b><small class="mono">${esc(maKH)}</small></div><div><span>Khoản vay</span><b>${esc(fmtNum(master&&master['KIM NGẠCH VAY']))} ${esc(master&&master['ĐỒNG TIỀN'])}</b><small>${hist.length} văn bản/hồ sơ liên quan</small></div></div>
