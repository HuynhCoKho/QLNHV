...
function openLendingForm(record) {
  ...
  openModal(edit?'Sửa khoản cho vay':'Thêm khoản cho vay', `<form id="lendingForm"><div class="form-grid">
    ...
    <div class="field span-2"><label>File văn bản xác nhận</label><input type="file" id="lendingFile" accept=".pdf,image/*"><span class="hint">File lưu riêng trên Google Drive; giữ nguyên file cũ nếu không chọn file mới.</span>${record.FILE?`<a href="${esc(record.FILE)}" target="_blank" rel="noopener">Mở file hiện tại</a>`:''}</div>
      ...
      el.querySelector('form').onsubmit=async e=>{e.preventDefault();const btn=el.querySelector('#saveLending');btn.disabled=true;try{const fd=new FormData(e.target),data={};LENDING_FIELDS.forEach(k=>data[k]=fd.get(k)||'');data['NGÀY VBXN']=toVNDate(fd.get('NGÀY VBXN'));data['KIM NGẠCH VAY']=parseNum(fd.get('KIM NGẠCH VAY'));data['DƯ NỢ']=parseNum(fd.get('DƯ NỢ'));data['HẾT NỢ']=fd.get('HẾT NỢ')==='true';data['CP BẢO LÃNH']=fd.get('CP BẢO LÃNH')==='true';data.FILE=record.FILE||'';const file=el.querySelector('#lendingFile').files[0];if(file)data.FILE=(await uploadLendingFile(file,data['MÃ SỐ KHOẢN CHO VAY'])).url;await apiPost(edit?'update':'create','ChoVay',data,edit?record['MÃ SỐ KHOẢN CHO VAY']:undefined);await reloadSheet('ChoVay');toast('Đã lưu khoản cho vay '+data['MÃ SỐ KHOẢN CHO VAY']);closeModal();renderChoVay();}catch(err){toast(err.message,true);btn.disabled=false;}};
    });
}
