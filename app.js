...
function printTinhThanhTKNTReport(tinh, groups) {
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Báo cáo TKNT - ${esc(tinh)}</title><style>@page{size:A4;margin:16mm}body{font:12px Arial,sans-serif;color:#111}h1{text-align:center;font-size:18px;margin:0 0 6px}h2{text-align:center;font-size:15px;margin:0 0 18px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #555;padding:6px;vertical-align:top}th{background:#eee;text-align:center}.num{text-align:center}.mono{font-family:monospace}.group{background:#e7f1ed;font-weight:bold}.group span{float:right;font-weight:normal}.summary{margin:0 0 12px;text-align:right}.foot{margin-top:12px;font-weight:bold}</style></head><body><h1>DANH SÁCH QUỐC GIA VÀ DOANH NGHIỆP CÓ TÀI KHOẢN NGOẠI TỆ</h1><h2>ĐANG HOẠT ĐỘNG - ${esc(String(tinh).toUpperCase())}</h2><div class="summary">${groups.length} quốc gia · ${companyCount} doanh nghiệp · ${accountCount} tài khoản</div><table><thead><tr><th>TT</th><th>QUỐC GIA / DOANH NGHIỆP</th><th>MÃ TÀI KHOẢN</th><th>SỐ TK</th></tr></thead><tbody>${groups.map((g,gi)=>`<tr class="group"><td class="num">${gi+1}</td><td colspan="2">${esc(g.country)} <span>${g.companies.length} doanh nghiệp</span></td><td class="num">${g.companies.reduce((n,x)=>n+x.accounts.length,0)}</td></tr>${g.companies.map((x,i)=>`<tr><td class="num">${gi+1}.${i+1}</td><td>${esc(x.customer.TenKhachHang)}<br><span class="mono">${esc(x.customer.MaKH)}</span></td><td class="mono">${x.accounts.map(a=>esc(a['MÃ TKNT'])).join('<br>')}</td><td class="num">${x.accounts.length}</td></tr>`).join('')}`).join('')}</tbody></table><div class="foot">Tổng cộng: ${groups.length} quốc gia</div><script>window.onload=()=>window.print()<\/script></body></html>`);
  w.document.close();
}
