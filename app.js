...
const ROUTE_SHEETS = {
  ...
  // HoSo không nằm trong danh sách này: trang hiện dữ liệu ngay, rồi tự tải
  // ngầm các hồ sơ có gắn mã khoản vay/cho vay/dự án phía sau (xem
  // renderChoVay/renderDTRNNN), không cần chờ và không cần toàn bộ sheet Hồ
  // sơ (~15.000 dòng).
...
function provinceTKNTData(tinh) {
  ...
  return DB.KhachHang.filter(k => provinceKey(k.DiaChiTinhTP) === key).map(k => {
    const accounts = DB.TKNHTONN.filter(t => {
      if (String(t['MÃ ĐƠN VỊ']) !== String(k.MaKH)) return false;
      const status = String(t['TRẠNG THÁI']).trim().toUpperCase();
      if (status !== 'ĐANG HOẠT ĐỘNG' && status !== 'CHƯA MỞ') return false;
      const licensed = parseTKNTVNDate(t['NGÀY GP']);
      return licensed && licensed <= cutoff;
    });
