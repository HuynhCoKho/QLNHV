...
function openTinhThanhTKNTPrintOptions(tinh, groups) {
  ...
  openModal('Thông tin xuất báo cáo - '+tinh,`<form id="provincePrintForm"><div class="form-grid"><div class="field"><label>Kỳ báo cáo *</label><input name="period" maxlength="6" inputmode="numeric" placeholder="Ví dụ: 202601" required></div><div class="field"><label>Chuyên viên lập biểu *</label><input name="maker" list="provinceMakerList" placeholder="Gõ mã hoặc tên" required><datalist id="provinceMakerList">${cvOptions}</datalist></div><div class="field"><label>Lãnh đạo ký *</label><input name="leader" list="provinceLeaderList" placeholder="Gõ mã hoặc tên" required><datalist id="provinceLeaderList">${cvOptions}</datalist></div><div class="field"><label>Chức danh lãnh đạo</label><input name="leaderTitle" value="TRƯỞNG PHÒNG"></div></div><div class="modal-foot"><button type="button" class="btn btn-outline" id="provincePrintCancel">Hủy</button><button class="btn btn-primary">In / Lưu PDF</button></div></form>`,el=>{el.querySelector('#provincePrintCancel').onclick=closeModal;el.querySelector('form').onsubmit=e=>{e.preventDefault();const fd=new FormData(e.target),period=fd.get('period').trim(),makerCode=fd.get('maker').split(' — ')[0].trim(),leaderCode=fd.get('leader').split(' — ')[0].trim(),maker=cvName(makerCode),leader=cvName(leaderCode);if(!/^\d{4}0[1-4]$/.test(period)){toast('Kỳ phải có dạng 202601 đến 202604.',true);return}if(!maker||!leader){toast('Vui lòng chọn đúng chuyên viên và lãnh đạo từ danh sách.',true);return}printTinhThanhTKNTReport(tinh,groups,{period,maker,leader,leaderTitle:fd.get('leaderTitle').trim()||'TRƯỞNG PHÒNG'})}});
}
